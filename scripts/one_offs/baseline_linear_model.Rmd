...
- For each geo, regress the last few weeks (population scaled) on the week number.
...
quantile_forecast <- map(-1:3, ~ forecaster_baseline_linear(epi_data, .x)) %>%
    bind_rows() %>%
...
p <- plot_forecasts(quantile_forecast, forecast_date, geo_type = "state", truth_data = truth_data, relevant_period = train_data) +
