...
ensemble_targets <- tar_map(
  ...
  # on the final forecast date. Write/validate/render bodies live in R/flu_outputs.R.
  tar_target(
  ...
  tar_target(
    ...
    command = if (!g_evaluation_mode) {
      flu_render_forecast_notebook(forecasts_and_ensembles, truth_data, forecast_date_int, forecast_report_rmd)
    }
  )
)
