---
title: "`r params$disease` `r params$target` `r params$averaging_method` score report"
...
options(width=500)
library(ggnewscale)
dbg <- function(...) write(paste0("[", format(Sys.time(), "%H:%M:%S"), "] ", ...), "/tmp/ongoing_score_debug.txt", append = TRUE)
dbg("setup chunk start")
recent_archive <- params$archive
dbg(paste("archive DT cols:", paste(names(recent_archive$DT), collapse = ", ")))
dbg(paste("is.data.table:", data.table::is.data.table(recent_archive$DT)))
if (params$target == "nssp") {
  recent_archive$DT <- rename(recent_archive$DT, value = nssp)
  dbg("renamed nssp -> value")
}
this_week <- round_date(Sys.Date() - 3, "week", 6)
dbg("setup chunk done")
```

# Recent scores {.tabset}
This "averaging method" refers to whether each forecaster's score is averaged over its own individually-reported set of keys and horizons or if it's averaged over a common set. The main difference is that for the "common" method, PyRenew is tossed out because it reports too few horizons, and we also stop evaluating horizon -1.

```{r adjusting data, echo=FALSE, message=FALSE}
averaging_method <- params$averaging_method %||% "individual"
if (averaging_method == "individual") {
  forecasts <- params$external_forecasts
  scores <- params$scores
} else if (averaging_method == "common") {
  forecasts <- params$external_forecasts %>%
    # Filter out PyRenew because they only report horizons 0 and 1
    filter(forecaster %nin% c("CFA_Pyrenew-Pyrenew_HE_Flu", "CFA_Pyrenew-Pyrenew_H_Flu")) %>%
    # Select forecasts for only horizons 1 to 4 for all forecasters
    filter(as.numeric(target_end_date - forecast_date) >= 0)
  scores <- params$scores %>%
    filter(forecaster %nin% c("CFA_Pyrenew-Pyrenew_HE_Flu", "CFA_Pyrenew-Pyrenew_H_Flu")) %>%
    filter(ahead >= 0)
} else {
...
score_summary <- function(scores) {
  ...
  scores %>%
    ...
    arrange(mean_wis) %>%
    select(forecaster, mean_wis, se_mean_wis, mean_wis_rate, se_mean_wis_rate, mean_cov_50, mean_cov_90, geomean_wis, n) %>%
    mutate(forecast_date = .env$forecast_date)
}
...
score_summary_8 <- scores %>%
  ...
  score_summary() %>% select(-forecast_date)
...
## Comparing with PyRenew last 8 weeks
```{r datatable_pyrenew, fig.height = 60, fig.width = 12, echo=FALSE, message=FALSE}
pyrenew_keys <- scores %>% filter(grepl("Pyrenew", forecaster)) %>% distinct(forecast_date, ahead, geo_value)
score_summary_pyrenew <- scores %>%
  filter(forecast_date >= this_week - 8 * 7) %>%
  right_join(pyrenew_keys) %>%
  score_summary() %>%
  select(-forecast_date)
datatable_function(score_summary_pyrenew)
```
...
```{r revisions, echo=FALSE, message=TRUE, out.width="100%"}
dbg("revisions chunk start")
recent_archive <- recent_archive %>%
  filter(this_week - time_value < 10*7)
recent_archive$time_type <- "day"
dbg(paste("post-filter archive DT cols:", paste(names(recent_archive$DT), collapse = ", ")))
dbg(paste("is.data.table post-filter:", data.table::is.data.table(recent_archive$DT)))
revision_sum <- recent_archive %>%
  epiprocess::revision_analysis(value, min_waiting_period = NULL)
dbg("revision_analysis done")
...
dbg(paste("all_revision_plots: is.data.table before mutate:", data.table::is.data.table(recent_archive$DT)))
recent_archive$DT %<>% mutate(geo_value = factor(geo_value, levels = av_re_spread$geo_value))
dbg(paste("all_revision_plots: is.data.table after mutate:", data.table::is.data.table(recent_archive$DT)))
recent_archive %>%
  autoplot("value") +
  facet_wrap(~geo_value, ncol = 3, scales = "free") +
  theme(strip.text.x = element_text(size = 8)) +
...
plotting_forecasts <- function(plotting_window, score_window, geo_score_order, n_plotting) {
  dbg(paste("plotting_forecasts called, target:", params$target, "n_plotting:", n_plotting))
  dbg(paste("recent_archive$DT is.data.table:", data.table::is.data.table(recent_archive$DT)))
  geo_score_order_2 <- geo_score_order[1:n_plotting]
  plotting_archive <- recent_archive
  plotting_archive$DT <- data.table::as.data.table(recent_archive$DT)
  dbg(paste("after as.data.table copy, is.data.table:", data.table::is.data.table(plotting_archive$DT)))
  plotting_archive$DT %<>%
    filter(geo_value %in% geo_score_order_2, time_value > plotting_window)
  dbg(paste("after filter, is.data.table:", data.table::is.data.table(plotting_archive$DT)))
  dbg(paste("after filter, cols:", paste(names(plotting_archive$DT), collapse = ", ")))
  if (params$target == "nhsn") {
    plotting_archive$DT %<>%
      left_join(state_census, by = join_by(geo_value == abbr)) %>%
      mutate(value = value * 1e5/ pop) %>%
      select(-pop)
    dbg(paste("after nhsn join, cols:", paste(names(plotting_archive$DT), collapse = ", ")))
  } else {
    forecasts %<>%
      mutate(
        value = value * 100
  ...
  plotting_archive$DT %<>% mutate(geo_value = factor(geo_value, levels = geo_score_order_2))
  data.table::setDT(plotting_archive$DT)
  data.table::setkeyv(plotting_archive$DT, c("geo_value", "time_value", "version"))
  dbg(paste("before autoplot, is.data.table:", data.table::is.data.table(plotting_archive$DT)))
  dbg(paste("before autoplot, key:", paste(data.table::key(plotting_archive$DT), collapse = ", ")))
  dbg(paste("before autoplot, cols:", paste(names(plotting_archive$DT), collapse = ", ")))
  ...
  plotting_archive %>%
    autoplot("value") +
    new_scale_color() +
    geom_ribbon(data = forecasts_to_plot, aes(x = time_value, ymin = `0.25`, ymax = `0.75`, fill = forecaster, group = forecast_date), alpha = 0.4, inherit.aes = FALSE) +
    geom_ribbon(data = forecasts_to_plot, aes(x = time_value, ymin = `0.1`, ymax = `0.9`, fill = forecaster, group = forecast_date), alpha = 0.4, inherit.aes = FALSE) +
    geom_ribbon(data = forecasts_to_plot, aes(x = time_value, ymin = `0.05`, ymax = `0.95`, fill = forecaster, group = forecast_date), alpha = 0.4, inherit.aes = FALSE) +
    geom_line(data = forecasts_to_plot, aes(x = time_value, y = `0.5`, group = forecast_date), alpha = 0.4, inherit.aes = FALSE) +
