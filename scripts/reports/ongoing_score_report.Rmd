...
plotting_forecasts <- function(plotting_window, score_window, geo_score_order, n_plotting) {
  ...
  finalized_check <- epix_as_of(plotting_archive, max(plotting_archive$DT$version))
  dbg(paste("finalized cols:", paste(names(finalized_check), collapse = ", ")))
  dbg(paste("finalized nrow:", nrow(finalized_check)))
  rm(finalized_check)
