...
our_forecasters <- c("linear", "windowed_seasonal", "windowed_seasonal_nssp", "climate_base", "climate_geo_agged", "climate_linear", "ensemble_windowed", "retro_submission", "CMU-TimeSeries", "CMU-TimeSeries-Calibrated", "seasonal_nssp_latest")
scores <- scores %>%
  mutate(
    forecaster =
      case_match(
...
datatable_function <- function(score_summary) {
  datatable(
    ...
    formatStyle("forecaster", target = c("cell"),
                textDecoration = styleEqual(c("CMU-TimeSeries", "CMU-TimeSeries-Calibrated"), c("underline", "underline"))) %>%
    formatStyle(
      "mean_wis",
...
sliding_ordered <- ordered_scores %>%
  ...
  slice(1)
baseline_only <- sliding_ordered %>% filter(forecaster == "CMU-TimeSeries")
...
top_5_and_CMU <- score_summary_short %>%
  pull(forecaster) %>%
  `[`(1:5) %>%
  append(c("CMU-TimeSeries", "CMU-TimeSeries-Calibrated")) %>%
  unique()
...
plotting_score <- function(sliding_plot, score_to_plot) {
  resulting_plot <- sliding_plot %>%
    ggplot(aes(x = forecast_date, y = {{ score_to_plot }}, color = forecaster, linewidth = ifelse(forecaster %in% c("CMU-TimeSeries", "CMU-TimeSeries-Calibrated"), 1, 0.5))) +
    geom_line() +
...
plotting_forecasts <- function(plotting_window, score_window, geo_score_order, n_plotting) {
  ...
  if (params$target == "nhsn") {
    plotting_archive$DT %<>%
      left_join(select(state_census, abbr, pop), by = join_by(geo_value == abbr)) %>%
