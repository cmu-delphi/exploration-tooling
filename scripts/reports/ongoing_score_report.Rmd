...
dbg(paste("params$target:", params$target))
dbg(paste("params$archive class:", paste(class(params$archive), collapse = ", ")))
...
dbg(paste("external_forecasts cols:", paste(names(params$external_forecasts), collapse = ", ")))
dbg(paste("scores cols:", paste(names(params$scores), collapse = ", ")))
...
dbg("adjusting data chunk start")
averaging_method <- params$averaging_method %||% "individual"
dbg(paste("averaging_method:", averaging_method))
...
dbg(paste("forecasts cols:", paste(names(forecasts), collapse = ", ")))
dbg(paste("scores cols:", paste(names(scores), collapse = ", ")))
...
dbg(paste("scores post-join cols:", paste(names(scores), collapse = ", ")))
Mean <- function(x) mean(x, na.rm = TRUE)
GeoMean <- function(x, offset = 0) exp(Mean(log(x + offset)))
dbg("adjusting data chunk done")
```
...
dbg(paste("revision_plots: filtered_archive DT cols:", paste(names(filtered_archive$DT), collapse = ", ")))
...
dbg(paste("all_revision_plots: recent_archive DT cols:", paste(names(recent_archive$DT), collapse = ", ")))
...
plotting_forecasts <- function(plotting_window, score_window, geo_score_order, n_plotting) {
  dbg(paste("plotting_forecasts called, target:", params$target))
  ...
  dbg(paste("plotting_archive DT cols:", paste(names(plotting_archive$DT), collapse = ", ")))
  if (params$target == "nhsn") {
    dbg(paste("state_census cols:", paste(names(state_census), collapse = ", ")))
    dbg(paste("geo_value class:", class(plotting_archive$DT$geo_value)))
    plotting_archive$DT %<>%
    left_join(state_census, by = join_by(geo_value == abbr)) %>%
    mutate(value = value * 1e5/ pop) %>%
    select(-pop)
    dbg(paste("after nhsn join, plotting_archive DT cols:", paste(names(plotting_archive$DT), collapse = ", ")))
  } else {
    forecasts %<>%
      mutate(
        value = value * 100
  ...
  forecasts_to_plot <- forecasts %>%
    ...
    ungroup()
  dbg(paste("forecasts_to_plot cols before pivot:", paste(names(forecasts_to_plot), collapse = ", ")))
  if (params$target == "nhsn") {
    forecasts_to_plot %<>%
      left_join(state_census, by = join_by(geo_value == abbr)) %>%
      mutate(value = value * 1e5/ pop)
    dbg(paste("forecasts_to_plot cols after nhsn join:", paste(names(forecasts_to_plot), collapse = ", ")))
  }
  forecasts_to_plot %<>%
