...
```{r}
hhs_forecast <- hhs_archive %>% epix_as_of(forecast_date)
all_forecasts <- forecast_aheads(\(x, ahead) scaled_pop(x, "hhs", ahead = ahead, trainer = linear_reg()))
default_geos <- c("ca", "fl", "ny", "pa", "tx")
plot_forecasts(all_forecasts, default_geos)
```
...
```{r}
hhs_forecast <- hhs_archive %>% epix_as_of(forecast_date)
all_linear_forecasts <- forecast_aheads(\(x, ahead) scaled_pop(x, "hhs", ahead = ahead, trainer = linear_reg()))
default_geos <- c("ca", "fl", "ny", "pa", "tx")
plot_forecasts(all_linear_forecasts, default_geos)
```
