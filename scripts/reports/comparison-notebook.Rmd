---
title: "`r params$forecaster_family`: evaluation on 2024/25 in 2025/26"
...
scores <- params$scores %>% filter(ahead %% 7 == 0)
...
p <- ggplot(
  ...
  geom_line(aes(color = forecaster, group = forecaster)) +
  geom_point(aes(color = forecaster, group = forecaster))
ensemble_scores <- normalized_df %>% filter(forecaster == ensemble_forecaster_name)
if (nrow(ensemble_scores) > 0) {
  p <- p +
    geom_line(
