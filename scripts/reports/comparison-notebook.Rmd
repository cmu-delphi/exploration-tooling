...
# i <- 1
# params <- list(
#   forecaster_family = tar_read(forecaster_families)[[i]],
#   forecaster_parameters = tar_read(forecaster_parameter_combinations)[[i]],
#   forecasts = tar_read(joined_forecasts) %>% filter(forecaster %in% c(tar_read(forecaster_parameter_combinations)[[i]]$id, outside_forecaster_subset)),
#   scores = tar_read(joined_scores) %>% filter(forecaster %in% c(tar_read(forecaster_parameter_combinations)[[i]]$id, outside_forecaster_subset)),
...
```{r}
if (params$disease == "flu") {
  plot_dates <- config$forecast_dates[seq(1, length(config$forecast_dates), by = 4)] + 3
} else {
  plot_dates <- config$forecast_dates[seq(1, length(config$forecast_dates), by = 4)] + 3
}
```
