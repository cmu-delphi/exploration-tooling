...
There is substantial underreporting behavior that is fairly consistent for a single geo.
It is likely usable for forecasting.
...
nhsn_archive_flu <- nhsn_archive_flu$DT %>% filter(time_value >= "2024-11-19") %>% as_epi_archive()
...
```{r}
nhsn_archive_flu %>% autoplot() + theme(strip.text.x = element_text(size = 2))
```
...
34%, so not a great track record overall.
...
Overall, it turns out our value substitutions did not actually help much.
...
```{r}
nhsn_archive_covid %>% autoplot() + theme(strip.text.x = element_text(size = 8))
```
