...
```{r}
# params <- list(forecaster_set = 1)
# Load the table of parameter combinations
s3load("covid_2023_forecaster_parameter_combinations.rds", bucket = "forecasting-team-data")
...
s3load(object = "covid_2023_joined_scores.rds", bucket = "forecasting-team-data")
scores <- covid_2023_joined_scores %>%
...
param_table <- forecaster_parameter_combinations_[[params$forecaster_set]] %>%
    select(-any_of(c("forecaster", "keys_to_ignore"))) %>%
    {
...
- The WIS plots are relative to the COVIDhub-baseline.
- The absolute error plots are also relative to the COVIDhub-baseline and are aggregated with the arithmetic mean.
- The dashed black line in all plots is the COVIDhub-ensemble.
...
s3load(object = "covid_2023_joined_forecasts.rds", bucket = "forecasting-team-data")
s3load(object = "covid_2023_truth_data.rds", bucket = "forecasting-team-data")
...
forecast_subset <- covid_2023_joined_forecasts %>%
    filter(
...
p <- ggplot(
    ...
    geom_line(
        data = covid_2023_truth_data %>%
            filter(geo_value %in% geo_vals, target_end_date >= min(plot_dates), target_end_date <= max(plot_dates)),
