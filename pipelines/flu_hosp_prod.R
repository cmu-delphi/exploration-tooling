...
if (!g_evaluation_mode) {
  stamp_default_weights("pipelines/flu_geo_exclusions.csv", g_forecast_dates[[1L]])
  stamp_default_weights("pipelines/flu_nssp_geo_exclusions.csv", g_forecast_dates[[1L]])
}
...
parameters_and_date_targets <- rlang::list2(
  ...
  tar_file(
    flu_geo_exclusions,
    command = "pipelines/flu_geo_exclusions.csv"
  ),
  tar_file(
    flu_nssp_geo_exclusions,
    command = "pipelines/flu_nssp_geo_exclusions.csv"
  ),
  tar_file(
...
g_ensemble_specs <- list(
  ...
  ensemble_mix = list(
    ...
    components = list(
      ...
      nssp = c("windowed_seasonal", "windowed_seasonal_extra_sources", "revision_aware")
    ),
    ...
    drop_negative_aheads_exempt = list(nhsn = c("revision_aware"), nssp = c("revision_aware")),
    apply_geo_exclusions = FALSE,
