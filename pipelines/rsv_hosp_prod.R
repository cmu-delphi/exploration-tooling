# The RSV Hospitalization Production Forecasting Pipeline.
...
set_targets_config()
g_aheads <- -1:3
g_submission_directory <- Sys.getenv("RSV_SUBMISSION_DIRECTORY", "cache")
...
# Forecaster grid — function definitions live in R/rsv_prod_forecasters.R.
ids <- c(
  "cdc_baseline",
  "linear",
...
g_forecaster_params_grid <- tibble(
  ...
  forecaster = rlang::syms(c(
    ...
    "g_rsv_linear",
    "g_rsv_linear_no_population_scale",
    "g_rsv_windowed_seasonal",
    "g_rsv_windowed_seasonal_extra_sources",
    "g_rsv_climate_base",
    "g_rsv_climate_geo_agged",
    "g_rsv_windowed_seasonal",
    "g_rsv_windowed_seasonal_extra_sources"
  )),
...
parameters_and_date_targets <- rlang::list2(
  ...
  tar_target(
    ...
    command = {
      up_to_date_nssp_state_archive("rsv")
    },
    cue = tar_cue("always")
  ),
  tar_target(
  ...
  tar_target(
    ...
    command = {
      ...
      nhsn_data <- nhsn_data %>%
        add_season_info() %>%
        mutate(
          geo_value = dplyr::if_else(geo_value == "usa", "us", geo_value),
          time_value = floor_date(time_value, "week", week_start = 7) + 3
        ) %>%
        filter(geo_value %nin% g_insufficient_data_geos)
      if (!grepl("latest", id)) {
        nhsn_data %<>%
          data_substitutions(rsv_data_substitutions, as.Date(forecast_generation_date_int))
      }
...
ensemble_targets <- tar_map(
  ...
  tar_target(
    ...
    command = {
      ...
      list(
        nhsn = make_weights(rsv_geo_exclusions),
        nssp = make_weights(rsv_nssp_geo_exclusions)
      )
    }
  ),
  tar_target(
  ...
  tar_target(
    ...
    command = {
      ar_nhsn <- forecast_filtered$nhsn %>%
        filter(forecaster %in% c("windowed_seasonal", "windowed_seasonal_extra_sources")) %>%
        filter(forecast_date < target_end_date) # rsv nhsn: drop neg aheads from AR
    ...
    command = {
      if (!g_backtest_mode && g_submission_directory != "cache") {
        ...
        nhsn_submission <- ensemble_mixture$nhsn %>%
          format_flusight(disease = "rsv")
        nssp_submission <- ensemble_mixture$nssp %>%
          format_flusight(disease = "rsv") %>%
          mutate(
            target = "wk inc rsv prop ed visits",
            value = value / 100
          )
      ...
      if (!g_backtest_mode) {
        ...
        rmarkdown::render(
          ...
          output_file = here::here(
            "reports",
            sprintf("%s_rsv_prod_on_%s.html", as.Date(forecast_date_int), as.Date(Sys.Date()))
          ),
          params = list(
            disease = "rsv",
            forecast_nhsn = forecasts_and_ensembles$nhsn %>% ungroup() %>% filter(forecaster != "climate_geo_agged"),
            forecast_nssp = forecasts_and_ensembles$nssp %>% ungroup() %>% filter(forecaster != "climate_geo_agged"),
...
joined_targets <- list2(
  ...
  tar_target(
    ...
    command = {
      score_forecasts(nhsn_latest_data, local_forecasts_and_ensembles_nhsn, "wk inc rsv hosp")
    }
  ),
  tar_target(
    ...
    command = {
      nssp_latest_data %>%
        ...
        score_forecasts(local_forecasts_and_ensembles_nssp %>% mutate(value = value / 100), "wk inc rsv prop ed visits")
    }
  )
)
    ...
    tar_target(
      ...
      command = {
        ...
        rmarkdown::render(
          ...
          output_file = here::here(
            "reports",
            sprintf("%s_rsv_nhsn_scoring.html", as.Date(Sys.Date()))
          ),
          params = list(
            disease = "rsv",
            target = "nhsn",
            external_forecasts = external_forecasts_full %>% filter(target == "wk inc rsv hosp") %>% select(-target),
    ...
    tar_target(
      ...
      command = {
        ...
        rmarkdown::render(
          ...
          output_file = here::here(
            "reports",
            sprintf("%s_rsv_nssp_scoring_individual.html", as.Date(Sys.Date()))
          ),
          params = list(
            disease = "rsv",
            target = "nssp",
            external_forecasts = external_forecasts_full %>% filter(target == "wk inc rsv prop ed visits") %>% select(-target),
        ...
        rmarkdown::render(
          ...
          output_file = here::here(
            "reports",
            sprintf("%s_rsv_nssp_scoring_common.html", as.Date(Sys.Date()))
          ),
          params = list(
            disease = "rsv",
            target = "nssp",
