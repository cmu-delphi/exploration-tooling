...
scaled_pop_seasonal <- function(epi_data,
  ...
  if (("flu" %in% seasonal_method) || ("covid" %in% seasonal_method)) {
    ...
    if (train_residual) {
      ...
      values_subtracted <- epi_data %>% mutate(epiweek = epiweek(time_value) %>% select(geo_value, source, epiweek, value = PC1) %>% distinct(geo_value, source, season_week, .keep_all = TRUE)
    }
  ...
  if ("climatological" %in% seasonal_method) {
    ...
    if (train_residual) {
      ...
      values_subtracted <- epi_data %>% mutate(epiweek = epiweek(time_value) %>% select(geo_value, source, epiweek, value = climate_median) %>% distinct(geo_value, source, epiweek, .keep_all = TRUE)
    }
  }
