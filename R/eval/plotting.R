...
# Sets the initial view to the past year on x and windowed per-geo max on y.
# x uses coord_cartesian (no clipping); y uses facetted_pos_scales (clips to
# windowed max, which is fine for a current-week forecast report).
apply_view_defaults <- function(g, forecast_data, truth_data, forecast_date, quantiles = c(0.8)) {
  ...
  ens_max <- forecast_data %>%
    filter(
      forecaster == "ensemble_mix",
      near(.data$quantile, max_q),
      .data$target_end_date >= one_year_ago
    ) %>%
  ...
  geo_max <- bind_rows(ens_max, truth_max) %>%
    ...
    arrange(.data$geo_value)

  y_scales <- lapply(geo_max$max_y, function(m) scale_y_continuous(limits = c(0, m)))

  g +
    coord_cartesian(xlim = c(one_year_ago, forecast_end), expand = FALSE) +
