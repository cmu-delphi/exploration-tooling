# Forecaster functions for the COVID hospitalization production pipeline.
# These are referenced by name via rlang::syms() in g_forecaster_params_grid.

g_covid_linear <- function(epi_data, ahead, extra_data, ...) {
  forecaster_baseline_linear(epi_data, ahead, ..., residual_tail = 0.97, residual_center = 0.097, no_intercept = TRUE)
}

g_covid_linear_no_population_scale <- function(epi_data, ahead, extra_data, ...) {
...
g_covid_climate_base <- function(epi_data, ahead, extra_data, ...) {
  climatological_model(epi_data, ahead, ...)
}

g_covid_climate_geo_agged <- function(epi_data, ahead, extra_data, ...) {
  climatological_model(epi_data, ahead, ..., geo_agg = TRUE)
}

g_covid_windowed_seasonal <- function(epi_data, ahead, extra_data, ...) {
  epi_data %>%
    scaled_pop_seasonal(
      outcome = "value",
      ahead = ahead * 7,
...
g_covid_windowed_seasonal_extra_sources <- function(epi_data, ahead, extra_data, ...) {
  epi_data %>%
